...
def random_mini_batches(X,Y,mini_batch_size=64,seed=0):
    ...
    # handle the end case(last mini-batch < mini-batch-size)
...
def initialize_velocity(parameters):
    ...
    Initialize the velocity as a python dictionary with
                            - key dW1 db1 ... dWL dbL
                            - values: numpy arrays of zeros of the same shape as the corresponding gradient/parameters
    ...
    """
    L = len(parameters) // 2 #  number of layers in the neural networs
    v = {}
...
def update_parameters_with_momentum(parameters,grads,v,beta,learning_rate):
    ...
    # Momentum update for each parameters
...
def update_parameters_with_adam(parameters,grads,v,s,t,learning_rate=0.01,beta1=0.9,beta2=0.999,epsilon=1e-8):
    ...
    :param s: Adam variable, moving average of the squared graident, python dictionary
    :param t:
    :param learning_rate: the learning rate, scalar
    ...
    :param epsilon: hyperparameter preventing division by zero in Adam update
    :return:
    parameters -- python dictionary containing your updated parameters
    v -- Adam variable, moving average of the first gradient, python dictionary
    s -- Adam variable, moving average of the second gradient, python dictionary
    """

    L = len(parameters) // 2
    v_corrected = {}   # initialize first moment estimate, python dictionary
    s_corrected = {}   # initialize second moment estimate, python dictionary

    # perform Adam update on all parameters
    for l in range(L):
        # moving average of the gradients Input : " v  grads  beta1 "  output " v "
        v["dW" + str(l + 1)] = beta1 * v["dW" + str(l + 1)] + (1 - beta1) * grads["dW" + str(l + 1)]
        v["db" + str(l + 1)] = beta1 * v["db" + str(l + 1)] + (1 - beta1) * grads["db" + str(l + 1)]

        # compute bias-corrected first moment estimate, Input  v beta1 t   output v_corrected
        v_corrected["dW" + str(l + 1)] = v["dW" + str(l + 1)] / (1 - np.power(beta1,t))
        v_corrected["db" + str(l + 1)] = v["db" + str(l + 1)] / (1 - np.power(beta1, t))

        # moving average of squared gradients, input s grads beta2  ouput s
...
def model(X,Y,layer_dims,optimizer,learning_rate=0.0007,mini_batch_size=64,beta=0.9,beta1=0.9,beta2=0.999,epsilon=1e-8,num_epochs=10000,print_cost=True):
    """
    3-layer neural network which can be run in different optimization modes
